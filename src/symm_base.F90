...
CONTAINS
   ...
   SUBROUTINE inverse_s()
     ...
     DO isym = 1, nsym
        ...
        DO jsym = 1, nsym
           ...
           !
        ENDDO
        IF (.NOT.found) CALL throw( 'inverse_s', ' Not a group')
     ENDDO
     !
   ...
   SUBROUTINE set_sym_bl()
     ...
     USE modcom, only : dmatrix_inverse
     !
     IMPLICIT NONE
     !
 ...
 !    CALL invmat( 3, rot, overlap )
     call dmatrix_inverse(rot,overlap,3)
     !
     nrot = 1
     !
     DO irot = 1, 32
   ...
   10   CONTINUE
     ...
     IF ( .NOT. is_group(nrot) ) THEN
        ...
        CALL info( 'set_sym_bl', 'NOTICE: Symmetry group for Bravais lattice &
   ...
   SUBROUTINE find_sym( nat, tau, ityp, magnetic_sym, m_loc, no_z_inv )
     ...
     symm: DO i = 1, 3 !emine: if it is not resolved in 3 steps it is sth else?
       ...
       IF ( .NOT. is_group( nsym ) ) THEN
          IF (i == 1) CALL info( 'find_sym', &
                         'Not a group! Trying with lower acceptance parameter...' )
          accep = accep * 0.5d0
          IF (i == 3) THEN
            CALL info( 'find_sym', 'Still not a group! symmetry disabled' )
            nsym = 1
          ENDIF
          CYCLE symm
       ELSE
          IF (i > 1) CALL info( 'find_sym', 'Symmetry operations form a group' )
          EXIT symm
       ENDIF
     ENDDO symm
     !
     ...
     DO irot = 1, nrot
        ...
        IF ( sym(irot) ) THEN
           ...
           DO na = 1, nat
              !
              nb = irt(irot,na)
              IF ( nb < 1 .OR. nb > nat ) CALL throw( 'check_mag_sym', &
                  'internal error: out-of-bound atomic index')
              !
   ...
   LOGICAL FUNCTION checksym( irot, nat, ityp, xau, rau, ft_ )
     ...
     DO na = 1, nat
        DO nb = 1, nat
           !
           IF( ityp (nb) == ityp (na) ) THEN
              checksym =  eqvect( rau(1,na), xau(1,nb), ft_ , epslat )
              IF ( checksym ) THEN
                 !
   ...
   SUBROUTINE checkallsym( nat, tau, ityp )
     ...
     DO isym = 1, nsym
        ...
        ENDDO
        IF ( ANY(ABS(sy) > eps1) ) &
            CALL throw( 'checkallsym', 'not orthogonal operation')
     ENDDO
     !
     ...
     !
     DO isym = 1,nsym
        IF ( .NOT.loksym(isym) ) CALL throw( 'checkallsym', &
             'the following symmetry operation is not satisfied  ')
     ENDDO
     !
     IF (ANY(.NOT.loksym(1:nsym) ) ) THEN
         !call symmetrize_at (nsym, s, invs, ft, irt, nat, tau, at, bg, &
         !                    alat, omega)
         CALL throw( 'checkallsym', &
         ...
         IF ( fractional_translations ) THEN
            IF (ityp(nb) == ityp(na) ) THEN
               ...
               IF ( sym (irot) .AND. &
                   ...
                   CALL throw( 'sgam_at_ifc', 'overlapping atoms')
            ENDIF
         ENDIF
      ENDDO
      !
